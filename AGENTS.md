...
**There is no backend.** No `output: 'server'`, no adapter, no API routes, and no server or database dependency — `package.json` carries seven deps, all build and test tooling. Everything under `src/data/` is compiled into static HTML at build time, so "the data layer" is a set of TypeScript constants plus Zod-validated JSON (`courses.json`, `schedules.json`), not a service. Three third-party services do the jobs a backend would, all reached from the browser and all env-gated: **Formspree** (form submissions), **Stripe Payment Links** (payment — `src/data/payments.ts`; ClassManager.pro was evaluated and dropped on 2026-08-21, no account was ever created), and **Relias Learning** (the student LMS behind Student Login). None are configured yet.

The core split: **editorial content lives in `src/data/courses.json` and `src/data/schedules.json`**, each validated against a Zod schema by a sibling loader (`courses.ts`, `schedules.ts`) that the rest of the site imports from; **the remaining `src/data/` modules stay typed TypeScript** (`site.ts`, `faqs.ts`, `testimonials.ts`, `payments.ts`, `compliance.ts`); **pages under `src/pages/` are presentation** that imports from those modules. To change prices, hours, contact info, nav links, or course copy, edit `src/data/` — not the pages. `src/pages/courses/[slug].astro` statically generates one page per entry in `courses.json`.
...
- **Env-gated integrations**: `src/data/payments.ts` reads `PUBLIC_STRIPE_PAYMENT_URL` and the per-course `PUBLIC_STRIPE_LINK_*` vars (`import.meta.env`); Formspree and GA are likewise env-gated. A priced course whose own link is unset gets no Pay button rather than the general link — charging the wrong amount is worse than offering no online payment. Missing vars must degrade gracefully (fallback messaging, no broken UI) — the accounts don't exist yet. In CI these come from GitHub Actions secrets (`.github/workflows/deploy.yml`).
...
- Launch is blocked on external inputs: Formspree ID, Stripe setup, DNS cutover, and a canonical public email — the live Weebly site uses a Gmail address while `src/data/site.ts` still has `headwaynursing@comcast.net`. Don't "fix" the email without the decision from Daniel/Janice.
- `public/images/testimonials/t3.png` is a byte-copy of `logo-banner.png`, not a real testimonial photo.
- `docs/HANDOFF.md` is the operator guide for Janice (the site owner); later phases aim at her editing content herself. Other docs: `docs/DEPLOY.md` (Cloudflare Pages + DNS), `docs/PAYMENTS.md` (Stripe Payment Links), `docs/QA.md` (pre-launch checklist).
